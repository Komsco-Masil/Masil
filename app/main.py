...
def ensure_store_public_data_columns() -> None:
    ...
    boolean_default = "0" if settings.DATABASE_URL.startswith("sqlite") else "false"
    migrations = {
        "nts_verified": f"ALTER TABLE stores ADD COLUMN nts_verified BOOLEAN NOT NULL DEFAULT {boolean_default}",
        "gift_card_verified": f"ALTER TABLE stores ADD COLUMN gift_card_verified BOOLEAN NOT NULL DEFAULT {boolean_default}",
